...
# Use a lower timeout for port scanning
PORT_TIMEOUT = 0.3
...
def get_software_version(url):
    try:
        resp = requests.get(url if url.startswith('http') else 'http://' + url, timeout=4, verify=False)
...
def search_cves_mitre(software, version):
    ...
    try:
        resp = requests.get(url, timeout=8, headers={'User-Agent': 'Mozilla/5.0'}, verify=False)
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python enhanced.py <url or url_list.txt>")
        sys.exit(1)
    ...
    print('Report saved to final_report.csv', flush=True)
